...
let rec type_type = function
  ...
  | PTident {id;loc} -> 
    try 
        Tstruct(Hashtbl.find structure_env id)
    with
...
and expr_desc env loc = function
  ...
  | PEbinop (op, e1, e2) -> (
      ...
      match op with
      | Beq | Bne ->
          if toto1.expr_desc = TEnil && toto2.expr_desc = TEnil then error loc "e1 ou e2 était nill"
          else (TEbinop (op,toto1, toto2), Tbool, false)
      | Badd | Bsub | Bmul | Bdiv | Bmod ->
          let a1, b1 = expr env e1 in
          let a2, b2 = expr env e2 in
          if a1.expr_typ = Tint && a1.expr_typ = Tint then (TEbinop (op, a1, a2), Tint, false)
          else error loc "tu opérates sur des non int"
      | Band | Bor ->
          let a1, b1 = expr env e1 in
          let a2, b2 = expr env e2 in
          if a1.expr_typ = Tbool && a1.expr_typ = Tbool then (TEbinop (op, a1, a2), Tbool, false)
          else error loc "tu logicionnes sur des non bool"
      | Blt | Ble | Bgt | Bge ->
        let a1, b1 = expr env e1 in
        let a2, b2 = expr env e2 in
        if a1.expr_typ = Tint && a1.expr_typ = Tint then (TEbinop (op, a1, a2), Tbool, false)
          else error loc "tu compares des non int")
  | PEunop (Uamp, e1) -> (* TODONE *) if (a_t_on_une_l_value e1) then (TEunop(Uamp, fst(expr env e1)), (fst(expr env e1)).expr_typ, false) 
...
let phase1 = function
  | PDstruct { (*TODONE *) ps_name = { id; loc } } ->
      ...
      if Hashtbl.mem structure_env dskldhjlsdqfjs.id then
        error loc "Déjà utilisé ton id mon coco"
      else
        let titi = { s_name = dskldhjlsdqfjs.id; s_fields = Hashtbl.create 3; s_size = 0 } in
        Hashtbl.add structure_env dskldhjlsdqfjs.id titi
  | PDfunction _ -> ()

  let rec sizeof = function (* TODONE *) 
...
let rec type_bien_forme = function
  |PTident {id; loc}-> id = "bool" || id = "int" || id = "string" || Hashtbl.mem structure_env id
  |PTptr t -> type_bien_forme t  

let phase2 = function
  | PDfunction { pf_name = { id; loc }; pf_params = pl; pf_typ = tyl } -> (* TODONE *) 
      let kljkksfdf = {id;loc} in

      if  id = "main" && pl = [] && tyl = [] then
          found_main := true
      else (if id = "main" then
        error loc "Ton main est mal défini mon coco")
      ;



      if Hashtbl.mem fonction_env kljkksfdf.id then
        error loc "Déjà utilisé ton id de ta fonction mon coco"
      else begin

        let new_pl = ref [] in
        let rec pl_to_new_pl l = (match l with
          |[]-> ()
          |p::q -> let x = new_var (fst p).id (fst p).loc (type_type (snd p)) in new_pl:=x::(!new_pl) ; pl_to_new_pl q 
        )
        in pl_to_new_pl pl ; 

        let new_tyl = ref [] in
        let rec tyl_to_new_tyl l = (match l with
          |[]-> ()
          |p::q -> let x =  (type_type p) in new_tyl:=x::(!new_tyl) ; tyl_to_new_tyl q 
        )
        in tyl_to_new_tyl tyl ;

        let titi = { fn_name = kljkksfdf.id; fn_params = !new_pl; fn_typ = !new_tyl} in
        ...
        let rec aux1 l = match l with 
          ...
          | p::q -> 
            if not(type_bien_forme p) then
              error loc "Ton type de var est mal formé mon coco"
            else
              aux1 q
        in aux1 tyl;
        ...
        let rec aux2 l = match l with
          ...
          | p::q -> 
            if Hashtbl.mem nom_variables_de_la_fonction (fst p).id then
              error (fst p).loc "Déjà utilisé ton id de ta var mon coco"
            else
              (Hashtbl.add nom_variables_de_la_fonction (fst p).id ();
              if not(type_bien_forme (snd p)) then
                error (fst p).loc "Ton type de var est mal formé mon coco"
              else
                aux2 q)
        in aux2 pl end
        
  | PDstruct { ps_name = { id ; loc }; ps_fields = fl } -> (* TODONE *) 
    let pqfjmqhf = {id;loc} in
    try
      let toto = Hashtbl.find structure_env pqfjmqhf.id in
      let rec aux l titi = match l with
        | [] -> titi
        | p::q -> 
          if Hashtbl.mem toto.s_fields (fst p).id then
            error (fst p).loc "Déjà utilisé ton id de ton champ mon coco"
          else
            (Hashtbl.add toto.s_fields (fst p).id {f_name = (fst p).id; f_typ = (type_type (snd p)); f_ofs = (titi/8);};
            if not(type_bien_forme (snd p)) then
              error (fst p).loc "Ton type de ton champ est mal formé mon coco"
            else
              aux q (titi + sizeof (type_type (snd p))))
      in toto.s_size <- aux fl 0
    with
      | Not_found -> error pqfjmqhf.loc ("Ta strucure est introuvable !")
...
let decl = function
  | PDfunction { pf_name = { id; loc }; pf_body = e; pf_typ = tyl } ->
      ...
      let titi = ref [] in
      let f = { fn_name = id; fn_params = !titi; fn_typ = List.map type_type tyl } in
      ...
      try
        ...
        let rec aux x =
          Hashtbl.iter (fun _ field -> match field.f_typ with
                                      | Tstruct a -> if a.s_name = id then error dummy_loc ("Ta strucure est récursive !")
                                                      else aux a.s_fields
                                      | _ -> (); ) x
        in aux toto.s_fields;
        TDstruct s
      with
        | Not_found -> error msqkfhmqo.loc ("Ta strucure est introuvable !")
